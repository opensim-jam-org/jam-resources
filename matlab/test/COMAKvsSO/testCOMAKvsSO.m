...
%% Perform Static Optimiation Using IK Constrained model
ModelVisualizer.addDirToGeometrySearchPaths('../../models/healthy/lenhart2015/Geometry');
ModelVisualizer.addDirToGeometrySearchPaths('../models/healthy/lenhart2015/Geometry');
ik_constrained_model_file = ...
    '../results/comak-inverse-kinematics/ik_constrained_model.osim';
so_result_dir = '../results/static_optimization';

% Remove Ligament and Contact Forces From Model
so_model = Model('./results/comak-inverse-kinematics/ik_constrained_model.osim');
% frc_set = so_model.getForceSet();
% nRemove = 0;
% % for i = 0:frc_set.getSize()-2
% %     frc =frc_set.get(i);
% % 
% %     if(strcmp(frc.getConcreteClassName(),'Blankevoort1991Ligament'))
% %         %frc_set.remove(i-nRemove);
% %         
% %         nRemove=nRemove+1;
% %     end
% %     if(i==0)
...
new_frc_set = ForceSet();
frc_set = so_model.getForceSet();

for i = 0:frc_set.getSize()-2
    frc =frc_set.get(i);

    if(strcmp(frc.getConcreteClassName(),'Millard2012EquilibriumMuscle'))
        new_frc_set.cloneAndAppend(frc);
    end
end

frc_set.clearAndDestroy();
so_model.set_ForceSet(new_frc_set);

so_model.print('./results/static_optimization/ik_constrained_no_passive_frc.osim');


% Perform Static Optimization
analyze = AnalyzeTool();
analyze.setModelFilename(ik_constrained_model_file);
% analyze.setModel(Model(ik_constrained_model_file));
analyze.setReplaceForceSet(false);
analyze.setForceSetFiles(ArrayStr('../../models/healthy/lenhart2015/lenhart2015_reserve_actuators.xml',1));
analyze.setResultsDir(so_result_dir);
analyze.setExternalLoadsFileName('../../models/healthy/experimental_data/motion_analysis/overground_17_ext_loads.xml');
analyze.setCoordinatesFileName('../results/comak/walking_states.sto');
analyze.setLowpassCutoffFrequency(6);
analyze.setStartTime(1.16);
analyze.setFinalTime(2.32);

so = StaticOptimization();
% so.setStartTime(1.16);
% so.setEndTime(2.32);
so.setName('SO')
so.setUseMusclePhysiology(false);

analysis_set = analyze.updAnalysisSet();
analysis_set.cloneAndAppend(so);

analyze.print('./inputs/SO_settings.xml');

so_analyze=AnalyzeTool('./inputs/SO_settings.xml');
so_analyze.run();

%     comak.set_coordinates_file(');
%     comak.set_external_loads_file(),
%     comak.set_results_directory(comak_result_dir);
%     comak.set_results_prefix(results_basename);
%     comak.set_replace_force_set(false);
%     comak.set_force_set_file();
% so.print(static_opt_file);
% AnalyzeTool(static_opt_file);

%% Plot Results
so_result = osimTableToStruct(TimeSeriesTable('./results/static_optimization/_SO_activation.sto'));
comak_result = osimTableToStruct(TimeSeriesTable('./results/comak/walking_activation.sto'));

figure('name','Quadriceps Activation')
subplot(2,2,1);hold on
plot(so_result.recfem_r)
plot(comak_result.a_forceset_recfem_r)
title('Rectus Femoris')
ylim([0 1])

subplot(2,2,2);hold on
plot(so_result.vasint_r)
plot(comak_result.a_forceset_vasint_r)
title('Vastus Int')
ylim([0 1])

subplot(2,2,3);hold on
plot(so_result.vaslat_r)
plot(comak_result.a_forceset_vaslat_r)
title('Vastus Lat')
ylim([0 1])

subplot(2,2,4);hold on
plot(so_result.vasmed_r)
plot(comak_result.a_forceset_vasmed_r)
title('Vastus Med')
ylim([0 1])
legend('comak','SO')
