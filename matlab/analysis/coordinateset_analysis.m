classdef coordinateset_analysis
    properties
        names
        data
    end
    methods
        function obj = coordinateset_analysis(model,h5_file)
        %==================================================================
        %CLASS CONSTRUCTER
        %------------------------------------------------------------------
        ...
        %(include extension)
        %==================================================================
            coordset_path = ['/' char(model.getName()) '/coordinateset'];
            ...
            for i=1:nCoords
                ...
                for k=1:nDataSet
                    ...
                    obj.data.(coord).(param) = h5read(h5_file,data_set_name);
...
%             hold on;
%             for i = 1:nMsl                   
%                 plot(obj.time,obj.data.Muscle.(muscle_names{i}).(param_name),'LineWidth',2);                
%             end
%             xlabel('time [s]')
%             ylabel(param_name)
%             legend(muscle_names)
